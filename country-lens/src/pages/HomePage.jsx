...
const HomePage = () => {
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      ...
      try {
        ...
        if (regionParam) {
          console.log("Fetching by region:", regionParam);
          const regionResults = await getCountriesByRegion(regionParam);
          setRegion(regionParam);
          results = regionResults;
        } else if (languageParam) {
          console.log("Fetching by language:", languageParam);
          const languageResults = await getCountriesByLanguage(languageParam);
  ...
  const applyAllFilters = useCallback(
    async (region, language, searchTerm) => {
      try {
        setLoading(true);
        let results = [];
